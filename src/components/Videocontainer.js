...
export default function Videocontainer() {
  ...
  const [chatsData, setchatsData] = useState([
    ...
    },
    {
      self: 0,
      message:
        "Start sharing group code " +
        localStorage.getItem("groupcode") +
        " and invite more people",
      username: "Bot",
      type: 1,
    },
    { self: 0, message: "How are you doing buddy!", username: "Bot", type: 1 },
  ]);
  ...
  const emitCode = (event) => {
    ...
    socket.emit("join", {
      ...
      sender_id: localStorage.getItem("user_id"),
      player_state: event.target.getPlayerState(),
      username: localStorage.getItem("username"),
    });
  };
    ...
    setTimeout(() => {
      if (videoElement.target.getPlayerState() == 2) {
        ...
        socket.emit("join", {
          ...
          emit_from_clint: event.target.getCurrentTime(),
          sender_id: localStorage.getItem("user_id"),
          player_state: videoElement.target.getPlayerState(),
          username: localStorage.getItem("username"),
        });
      }
    }, "250");
  ...
  const statechange = (event) => {
    ...
    would be some time  */
    if (
      current_time - isselfstaterun < 4000 &&
      event.data == 1 &&
      videoElement.target.getCurrentTime() >= 1 &&
      current_time - pagejustloaded > 4000
    ) {
      videoElement.target.pauseVideo();
      setchatsData(function (chatsData) {
        let data_msg = chatsData.filter(function (item) {
          return (
            item.message !==
            "Please do not try to hit pause button continous. try hit after some seconds"
          );
        });
        return data_msg;
      });
      ...
      if (
        isNaN(current_time - got_the_socket_io_event) ||
        current_time - got_the_socket_io_event > 1000
      ) {
        let data_state =
          time_gap_on_request <= 1 && event.data != 3 ? 1 : event.data;
        if (data_state === 1) {
        ...
        event.data == 1 &&
        current_time - got_the_socket_io_event > 550
      ) {
        socket.emit("join", {
          group_code: localStorage.getItem("groupcode"),
          emit_from_clint: event.target.getCurrentTime(),
  ...
  useEffect(() => {
    offset > 100
      ? setfixedOrnot({ position: "fixed" })
      : setfixedOrnot({ position: "absolute" });
  }, [offset]);
          ...
          <div>
            ...
            <div className={chatheight} style={{ width: videoWidth.width }}>
              <div className="col-md-12 ">
                <Chatbox
                  isActive={isActive}
                  onuserStopChat={onuserStopChat}
                  chatMessageonkeyup={chatMessageonkeyup}
